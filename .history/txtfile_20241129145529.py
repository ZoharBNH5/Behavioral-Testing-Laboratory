...
# Remove the 4th trial for C2R
filtered_animal_vectors = {}
for animal, vector in animal_vectors.items():
    if len(vector) >= 5:
        if animal == "C2R":
            # Exclude the 4th trial
            filtered_animal_vectors[animal] = [vector[0], vector[1], vector[2], None, vector[4]]
        else:
...
for i, (animal, vector) in enumerate(filtered_animal_vectors.items()):
    ...
    trials = [j + 1 for j in range(len(vector)) if vector[j] is not None]
    filtered_durations = [v for v in vector if v is not None]

    # Perform linear regression
    slope, intercept, _, _, _ = linregress(trials, filtered_durations)
    regression_line = [slope * x + intercept for x in trials]

    # Plot the data points and regression line
    plt.plot(trials, filtered_durations, 'o-', label=f'{animal} (Data)', color=colors[i % len(colors)])
