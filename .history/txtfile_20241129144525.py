...
for animal, vector in animal_vectors.items():
    if len(vector) >= 5:  # Ensure there are enough trials
        improvement = ((vector[0] - vector[-1]) / vector[0]) * 100
        improvement_percentages[animal] = improvement
        print(f"{animal}: Improvement = {improvement:.2f}%, Learning = {'Yes' if improvement > 0 else 'No'}")
