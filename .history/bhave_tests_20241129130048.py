...
# Define bins (2-minute intervals over 10 minutes)
    bin_edges = np.arange(0, 600 + 120, 150)  # Bins of 2 minutes (120 seconds)
...
# Calculate Thigmotaxis per bin
    ...
    plt.title(f'Thigmotaxis Over Time ({mouse_num})', fontsize=14)
    plt.xlabel('Time (seconds)', fontsize=12)
    plt.ylabel('Thigmotaxis (%)', fontsize=12)
    plt.xticks(bin_centers, labels=[f'{int(edge)}-{int(edge+120)}' for edge in bin_edges[:-1]])
